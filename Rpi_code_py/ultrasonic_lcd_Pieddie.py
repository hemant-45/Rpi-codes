...
IO.setup(17,IO.OUT)    # RS pin of lcd
IO.setup(27,IO.OUT)    # EN pin of lcd
IO.setup(24,IO.OUT)    # D0 pin of lcd
IO.setup(23,IO.OUT)    # D1 pin of lcd
IO.setup(18,IO.OUT)    # D2 pin of lcd
IO.setup(26,IO.OUT)    # D3 pin of lcd
IO.setup(5,IO.OUT)     # D4 pin of lcd
IO.setup(6,IO.OUT)     # D5 pin of lcd
IO.setup(13,IO.OUT)    # D6 pin of lcd
IO.setup(19,IO.OUT)    # D7 pin of lcd
IO.setup(21,IO.OUT)    # TRIGGER pin of Ultrasonic 
IO.setup(16,IO.IN)     # ECHO pin of Ultrasonic  
def send_a_command (command):  #steps for sending a command to 16x2 LCD
    pin=command
    PORT(pin);
    IO.output(17,0)    #PORTD&= ~(1<<RS); 
    IO.output(27,1)    #PORTD|= (1<<E);
    time.sleep(0.001)  #_delay_ms(50);
    IO.output(27,0)    #PORTD&= ~(1<<E);
    pin=0
    PORT(pin); 
def send_a_character (character):  #steps for sending a character to 16x2 LCD
    pin=character
    PORT(pin);
    IO.output(17,1)    #PORTD|= (1<<RS);
    IO.output(27,1)    #PORTD|= (1<<E);
    time.sleep(0.001)  #_delay_ms(50);
    IO.output(27,0)    #PORTD&= ~(1<<E);
    pin=0
...
while 1:
    ...
    if(distance>400):                  #If distance is more than 400cm, just print 400+ on LCD
         send_a_command(0x80 + 0);
         send_a_string ("Dist= 400+ cm" % (distance) );
         time.sleep(0.15)

        
    if(distance<=0):                  #If distance is more than 400cm, just print 400+ on LCD
         send_a_command(0x80 +0 );
         send_a_string ("Dist= " % (distance));
         time.sleep(0.15)
